...
public class ALG4_GRAY_1 {
    ...
    JFrame frame_bs = new JFrame("Recovered Stego image marked in red and white");
    ...
    JFrame frame_bs1 = new JFrame("Recovered Stego image  ");
    Panel panel_bs1 = new ShowImage(oi.getDirectory()+"Recoverd_gray_new.png");
...
ij.io.OpenDialog ai=new ij.io.OpenDialog("Select authentication image",
                  "E:\\DATA\\YES_MAN\\JAVA_IMPL\\",
                  "lenna.bmp");
 //public static String nn="Authentication.png";
    BufferedImage image_oii=null;
   File f_oii=new File(ai.getDirectory()+ai.getFileName());
         try {
             image_oii= ImageIO.read(f_oii);
         } catch (IOException ex) {
             Logger.getLogger(ALG4_GRAY.class.getName()).log(Level.SEVERE, null, ex);
         }
     ...
     BufferedImage gray_recovered_new =new BufferedImage(gray_i.getWidth(), gray_i.getHeight(),  BufferedImage.TYPE_INT_ARGB);
    ...
    // G[0]=(parseInt1+parseInt2/(100000.0));   
     ...
     G[0]=(parseInt1+(parseInt2*Math.pow(10, -5.0)));     
         try {  
             sCurrentLine = br.readLine();
         } catch (IOException ex) {
             Logger.getLogger(ALGO_41.class.getName()).log(Level.SEVERE, null, ex);
     ...
     G[1]=(parseInt1+(parseInt2*Math.pow(10, -5.0))); 
     ...
     int k=0;
        ...
        for (int cx=0;cx<image_oi.getWidth();cx++) {          
            for (int cy=0;cy<image_oi.getHeight();cy++) {
                ...
                gray_recovered_new.setRGB(cx, cy, rgb);
 ...
 //System.out.println("\n\n\tProcessing each block of alpha and binary to compute authentication signals a1,a2 and a1',a2' respectively........."); 
 //System.out.println("\n\tMarking each block of binary as untampered, if a1a2==a1'a2' and tampered otherwise........\n");
...
boolean p[]=new boolean[6];
 ...
 for (int j=0; j<lim_c  && j%3==0 ;j=j+3)
     for(int i=0; i<lim_r  && i%2==0;i=i+2) 
      ...
      //System.out.println("\nExtracting d and c1 using shamir recovery with (1,q[1]),(2,q[2])");
       ...
       //4.3.3 covert d to 4 bit bianry value and c1 into 4 bit binary value and combine them to 8 bit value.
       //take first two bits of it as s=a1a2
           //System.out.println("\td="+_d+"\tc1="+_c1);
       ...
       //take first two bits of it as s=a1a2);
           toBinary tb=new toBinary(d_c1);
            //System.out.println("\td_c1="+d_c1+"\tNo.of bits in d_c1="+tb.size);
           int tb1[]=new int[8];
           k=0;
           ...
           //System.out.print(tb1[kk]+" ");
           }
           //System.out.print("\nTaking first two bits of 8 bit representation of d_c1 as authentication signals s=a1a2:");
           //System.out.print("\n\ta1="+tb1[0]+"\t"+"a2="+tb1[1]);
           //4.4 from binary plane find two bit authentication signal s'=a1'a2'with a1'=p1 xor p2 xor p3 and a2'=p4 xor p5 xor p6
           //System.out.println("\n\nFrom Current binarized block of stego image finding two bit authentication signal s'=a1'a2'with a1'=p1 xor p2 xor p3 and a2'=p4 xor p5 xor p6........");
     ...
     //4.5 match s1 and s' if mismatch occurs mark currnt binary block and color image blok and all the six shares p1 to p6 as tampered
           ...
           //System.out.println("\ta1'="+s_[0]+"\ta2'="+s_[1]);
           //System.out.println("\ta1="+s[0]+"\ta2="+s[1]+"\ta1'="+s_[0]+"\ta2'="+s_[1]);
 ...
 { 
   ...
   { 
      ...
      ////System.out.println("\n\tCurrent Tampered Gray Block's six pixels values before recovery process:\n");
      ...
      {q[ii]=q1[ii]-238;  
       ...
       //take first two bits of it as s=a1a2
           ...
           int bi[]=new int[6];
              ...
              */
              //System.out.println("\n\n\t Recovery Process:");
              //System.out.println("\nIf b[i]=0 set y[i]=g1 else set y[i]=g2 where y[i] is a current gray level block's ith pixel and i=0,1,2,3,4,5 and mark block as repaired ");
              //System.out.println("\n\tCurrent Tampered Gray Block's six pixels values after recovery process:\n");
              ll=0;
              for(int mn=2;mn<=7;mn++){
                   bi[ll]=tb1[mn];
                   if(bi[ll]==0){
                   //System.out.print("\ty["+(mn-2)+"]="+(int)G[0]);
                   }
                   else
                   {
                       //System.out.print("\ty["+(mn-2)+"]="+(int)G[1]);
                   }
                   ll++;
              }
              ...
              if(bi[0]==0)
              {
                  ...
                  blue=(int)G[0]; 
                  rgb=(alpha << 24) | (red << 16) | (green << 8) | blue;
                  gray_recovered_new.setRGB(j, i, rgb);
             ...
             if(bi[1]==0)
              {
                  ...
                  blue=(int)G[0];
                  rgb=(alpha << 24) | (red << 16) | (green << 8) | blue;
                  gray_recovered_new.setRGB(j,i+1, rgb);
              ...
              if(bi[2]==0)
              {
                  ...
                  blue=(int)G[0];
                  rgb=(alpha << 24) | (red << 16) | (green << 8) | blue;
                  gray_recovered_new.setRGB(j+1,i, rgb);
              }
              else
              {     
                  alpha=AI[j+1][i];
              ...
              {   
                  ...
                  blue=(int)G[0];
                  rgb=(alpha << 24) | (red << 16) | (green << 8) | blue;
                  gray_recovered_new.setRGB(j+1,i+1, rgb);
              ...
              if(bi[4]==0)
              {
                 ...
                 red=(int)G[0];        
                  ...
                  blue=(int)G[0];
                  rgb=(alpha << 24) | (red << 16) | (green << 8) | blue;
                  gray_recovered_new.setRGB(j+2,i, rgb);
              }
              else
              {    
                  alpha=AI[j+2][i];
              ...
              if(bi[5]==0)
              {
                  ...
                  blue=(int)G[0];
                  rgb=(alpha << 24) | (red << 16) | (green << 8) | blue;
                  gray_recovered_new.setRGB(j+2,i+1, rgb);
  ...
  //save modified PNG image color
      ...
      try {     
          ImageIO.write(gray_recovered_new, "png", new File(oi.getDirectory()+"Recoverd_gray_new.png"));
      }catch (IOException e) {} 
      
        try {     
          ImageIO.write(gray_tampered, "png", new File(oi.getDirectory()+"Tampered_gray.png"));
      }catch (IOException e) {} 
       
//int rep=0,nrep=0,ntampblk=0,noofuntamp=0;    
  // blk_no=0;
        
  //do stistical analysis:      
   System.out.println("\n\t Number of Tampered Blocks detected="+ntampblk);
   System.out.println("\n\t Number of Tampered Blocks Repaired="+rep);
  // System.out.println("\n\tRepair ratio=Number of Tampered Blocks Repaired/Number of Tampered Blocks detected="+(rep/ntampblk));
   System.out.println("\n\tTotal Number of blocks="+blk_no);
