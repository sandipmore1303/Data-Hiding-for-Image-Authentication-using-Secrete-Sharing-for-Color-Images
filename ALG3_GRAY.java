...
public class ALG3_GRAY {
    ...
    public static void main(String args[]) throws IOException {
        ...
        try {
            ImageIO.write(gray_with_alpha, "png", new File(oi.getDirectory() + "Original_Gray.png"));
        } catch (IOException e) {
        }

        //1.4 convert to binary     
        int data[] = new int[lim_c * lim_r];
        int Gray_Image_original[][] = new int[lim_c][lim_r];
        int Alpha_Image_original[][] = new int[lim_c][lim_r];
        int Binary_Image_original[][] = new int[lim_c][lim_r];
        int Alpha_Image_mod[][] = new int[lim_c][lim_r];
        int data_2D[][]= new int[lim_c][lim_r];
        k = 0;
        ...
        for (int cx = 0; cx < lim_c; cx++) {
            for (int cy = 0; cy < lim_r; cy++) {
                ...
                green = (rgb & 0x0000FF00) >>> 8;
                blue = (rgb & 0x000000FF) >>> 0;
                data_2D[cx][cy]=(red + green + blue) / 3;
       ...
       k = 0; 
        int data_1D[]= new int[lim_c*lim_r];
        for (int cx = 0; cx < lim_c; cx++) {
            for (int cy = 0; cy < lim_r; cy++) {
             data_1D[k++]=data_2D[cx][cy];   
            }
        }
        ...
        //for (int i = 0; i < data_1D.length; i++) {
         //   histogram[data_1D[i]] = histogram[data_1D[i]] + 1;
        //}
        G = Moments(histogram);
        ...
        try {
            ImageIO.write(Binary_i, "png", new File(oi.getDirectory() + "Original_Binary.png"));
        } catch (IOException e) {
        }
        ...
        for (int j = 0; j < lim_c && j % 3 == 0; j = j + 3) {
            for (int i = 0; i < lim_r && i % 2 == 0; i = i + 2) {
                ...
                System.out.println("Two Authentication signals computed from six binary pixels of current Binary block (to be embedded in alpha plane):");
                System.out.println("s[0]=a1=p[0] XOR p[1] XOR p[2]="+a1+"\ts[1]=a2=p[3] XOR p[4] XOR p[5]="+a2);
                ...
                int d = 0, c1 = 0;
                d = (m1[0] * 2 * 2 * 2 + m1[1] * 2 * 2 + m1[2] * 2 + m1[3] * 1);
                c1 = (m2[0] * 2 * 2 * 2 + m2[1] * 2 * 2 + m2[2] * 2 + m2[3] * 1);
                int x[] = new int[6];
                int q[] = new int[6];
                int q1[] = new int[6];
                int F[] = new int[6];
                System.out.println(("d=m1=(a1 * 2 * 2 * 2 + a2 * 2 * 2 + p[0] * 2 + p[1] * 1)="+d+"\tc1=m2=(p[2] * 2 * 2 * 2 + p[3] * 2 * 2 + p[4] * 2 + p[5] * 1)="+c1));
                System.out.println("six partial shares from current Binary block computed using Shamir secrete share generation by using eqn:q[i] = ((d + c1 * (i+1)) % 17),i=0,1,2,..5;(to be embedded in alpha plane):");
                ...
                System.out.println("six partial shares for current alpha block  by using eqn: q1[i]=q[i]+238,i=0,1,2,..5;\n(first two shares will be retained,remaining four shares will be mapped to other blocks using key K ):");
                for (int mmm = 0; mmm < 6; mmm++) {
                     
                    System.out.println("q1["+mmm+"]="+q1[mmm] );
                }
        ...
        BufferedImage Authentication = new BufferedImage(gray_i.getWidth(), gray_i.getHeight(), BufferedImage.TYPE_INT_ARGB);
        //stego_binary=Binary_i;
        BufferedImage Authentication_data = new BufferedImage(gray_i.getWidth(), gray_i.getHeight(), BufferedImage.TYPE_INT_ARGB);
        ...
        for (int cy = 0; cy < lim_c; cy++) {
            for (int cx = 0; cx < lim_r; cx++) {
                ...
                stego_binary.setRGB(cy, cx, rgb);
                 Authentication.setRGB(cy, cx, (alpha << 24) | (alpha << 16) | (alpha << 8) | alpha);

                alpha = 255;
                red = SDATA[cy][cx]+238;
                green = SDATA[cy][cx]+238;
                blue = SDATA[cy][cx]+238;
                rgb = (alpha << 24) | (red << 16) | (green << 8) | blue;

                Authentication_data.setRGB(cy, cx, rgb);
        ...
        //}catch (IOException e) {} 
         ...
         try {
            ImageIO.write(Authentication_data, "png", new File(oi.getDirectory() + "Authentication_data.png"));
        } catch (IOException e) {}
        ...
        try {
            ImageIO.write(Authentication, "png", new File(oi.getDirectory() + "Authentication.png"));
        } catch (IOException e) {
        }
        try {
            ImageIO.write(Authentication_data, "png", new File(oi.getDirectory() + "Authentication_data.png"));
        ...
        //PSNR TMP = new PSNR();
        //double psnr = PSNR.psnr(gray_i, stego_gray);
