...
public class RGBStackMerge_new   {
	...
	private static  boolean ignoreLuts;
	private static  boolean autoFillDisabled;
	private static  String firstChannelName;
	...
	public static void mergeStacks() {
		...
		int[] wList = WindowManager.getIDList();
                ImagePlus imp[]=new ImagePlus[3];
                imp[0]=ij.IJ.openImage("gray_recovered_r.png");
                imp[1]=ij.IJ.openImage("gray_recovered_g.png");
                imp[2]=ij.IJ.openImage("gray_recovered_b.png");
		...
		for (int i=0; i<wList.length; i++) {
			//imp[i] = WindowManager.getImage(wList[i]);
			titles[i] = imp!=null?imp[i].getTitle():"";
		}
		...
		imp2.show();
               // ij.IJ.save(imp2, "recovered.bmp");
	 }
	 
	 private static String[] getInitialNames(String[] titles) {
	 ...
	 private static String getName(int channel, String[] titles) {
	 	if (autoFillDisabled)
	 		return none;
	...
	public static ImagePlus mergeHyperstacks(ImagePlus[] images, boolean keep) {
		int n = images.length;
		int channels = 0;
		for (int i=0; i<n; i++) {
	...
	private static boolean isDuplicate(int index, ImagePlus[] images) {
		int count = 0;
	...
	public static ImageStack mergeStacks(int w, int h, int d, ImageStack red, ImageStack green, ImageStack blue, boolean keep) {
	...
	private static ImagePlus mergeUsingRGBProjection(ImagePlus[] images, boolean createComposite) {
	...
	static byte[] getPixels(ImageStack stack, int slice, int color) {
		 if (stack==null)
			return blank;
